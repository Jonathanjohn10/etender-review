...
[ -f review/comments.json ] || curl -fsS "$SITE/api/comments" -o review/comments.json
...
curl -fsS -X POST -H 'content-type: application/json' --data "$BODY" "$SITE/api/comments" -o review/comments.json
if [ "$RESOLVE" = 1 ]; then
